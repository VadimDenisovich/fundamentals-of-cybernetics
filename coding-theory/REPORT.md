...
> **Что такое «валидационный вектор №$N$».** Здесь и далее это отсылка к набору из пяти эталонных проверок с заранее известным правильным ответом (полный список — §2.4), которыми подтверждается корректность реализации. Задание требует валидацию на 3–5 таких векторах.
...
**Процедура Чиеня** — перебор $j = 0,\dots,6$, корни $\Lambda(\alpha^{-j}) = 0$. Здесь $\alpha^{-j}$ — это **обратный элемент**, а не «отрицательная степень»: показатели берутся по модулю $7$ (так как $\alpha^7 = 1$), поэтому $\alpha^{-2} = \alpha^{5}$, $\alpha^{-5} = \alpha^{2}$ и в таблице стоят положительные степени. Сам номер позиции $j$ неотрицателен ($j = 0,\dots,6$); минус относится к взятию обратного локатора в формуле Чиеня.
...
Двоичный BCH-код в узком смысле (корни начинаются с $\alpha^1$) длины $n = 7$ (эквивалент кода Хэмминга). Порождающий многочлен — минимальный многочлен элемента $\alpha$: $g(x) = x^3 + x + 1$, $n - k = 3$, $k = 4$. Корни $\alpha, \alpha^2, \alpha^4$ (среди них $\alpha, \alpha^2$ подряд $\Rightarrow d \ge 3$).
...
> **Согласование реализаций.** По умолчанию `galois.ReedSolomon` использует первый корень $\alpha^1$ (в узком смысле, $c=1$), а `reedsolo` — $\alpha^0$ ($\mathit{fcr}=0$). Для побайтовой идентичности кодовых слов в `reedsolo` задано $\mathit{fcr}=1$; идентичность проверена *валидационным вектором №4*.
...
$N_{\text{blocks}} = 10^5$ для направления Б обеспечивает значимость при $\mathrm{FER}\sim 10^{-3}$. Поясним: $10^{-3}$ — это *доля* (вероятность) ошибки на кадр, а ожидаемое *число* ошибочных кадров равно $N_{\text{blocks}}\cdot\mathrm{FER} = 10^5 \cdot 10^{-3} = 100$. Ста событий достаточно для узкого доверительного интервала (при малом числе событий оценка FER неустойчива).

Для направления А выбрано $N_{\text{blocks}} = 10^4$. Условие $N \ge 10^4$ взято из формулировки гипотезы А2 (§3.1) — это минимум, при котором она ожидает заметную разницу в скорости. Больший $N$ не нужен: декодер `reedsolo` на чистом Python сделал бы $10^5$ блоков многочасовыми без улучшения выводов — сравнение идёт в области «колена», где $\mathrm{FER} \ge 10^{-2}$ (уже $\ge 100$ ошибочных кадров на точку).
...
Время измеряется только для операции декодирования; JIT-компиляция `galois` выполняется до замеров (предварительный прогрев).
...
Проверяются две гипотезы направления А (нумерация — как в списке предложенных в задании формулировок):
...
Проверяются две гипотезы направления Б (нумерация — как в задании; гипотеза Б2 о сдвиге «колена» при изменении $t$ не рассматривается):
